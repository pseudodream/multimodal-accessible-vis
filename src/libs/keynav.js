...
export const getFieldName = (spec, scaleName) => {
  ...
  if (isDataRef(scale.domain)) {
    if (scale.domain.field) {
      // string field name // TODO: what if it's a data ref?
    ...
    if (scale.domain.fields) {
      //scale.domain.fields[0].split("_")[0]
...
export const getEncodingTitle = (spec, scaleName) => {
  switch (scaleName) {
    ...
    case "color":
      return spec.legends.find((d) =>
        Object.keys(d).some((p) => d[p] === scaleName)
      ).title;
  }
};
export const getEncodingType = (spec, scaleName) => {
  // does it have a legend?
  const hasLegend = spec.legends.some((d) =>
    Object.keys(d).some((p) => d[p] === scaleName)
  );
  switch (scaleName) {
    case "x":
...
export const navtree = async (spec, options = {}) => {
  ...
  const datumDesc = options.datumDesc
    ? options.datumDesc
    : (datum, index, field, group, spec) => {
        const otherFields = spec.scales
          .map((d) => getFieldName(spec, d.name))
          .filter((d) => d != field);
        // console.log(field,otherFields, formatters);
        return `${index + 1} of ${group.length} items. ${field}: ${
          formatters[field] ? formatters[field](datum[field]) : datum[field]
        }, ${otherFields
          .map(
            (otherField) =>
              `${otherField}: ${
                formatters[otherField]
                  ? formatters[otherField](datum[otherField])
                  : datum[otherField]
              }`
          )
          .join(", ")}`;
  ...
  const categorySubgroupDesc = (
    ...
    return `${index + 1} of ${
      parentGroup.length
    } categories, ${field}: ${value}, ${group.length} values`;
  };

  const categoryGroupDesc = (type, title, domain, field, spec) => {
    return `${type} titled ${title} with categories starting from ${
      domain[0]
    } to ${domain[domain.length - 1]}`;
  };

  const numericSubgroupDesc = (
    group,
    field,
    extent,
    index,
    ...
    //TODO: add summary
    return `${index + 1} of ${parentGroup.length} ${field} ranges, from ${
      formatters[field] ? formatters[field](extent[0]) : extent[0]
    } to ${formatters[field] ? formatters[field](extent[1]) : extent[1]}, ${
      group.length
    } values`;
  };
  ...
  const numericGroupDesc = (type, title, domain, field, spec) => {
    //TODO: add summary
    return `${type} titled ${title} with categories starting from ${
      formatters[field] ? formatters[field](domain[0]) : domain[0]
    } to ${
      formatters[field]
        ? formatters[field](domain[domain.length - 1])
  ...
  const createCategorySubgroupNode = (
    group,
    field,
    value,
    index,
    ...
    const node = {
      ...
      selection: group,
      lastVisitedChild: null,
      description: categorySubgroupDesc(
        group,
        field,
        value,
        index,
    ...
    // level - 3
    node.children = group.map((d, i) =>
      createDatumNode(d, i, field, node, spec)
    );

    return node;
  };

  const createCategoryEncodingNode = (
    domain,
    type,
    title,
    field,
    ...
    encodingNode.description = categoryGroupDesc(
      type,
      title,
      domain,
      field,
    ...
    if (expand) {
      ...
      encodingNode.children = domain.map((group, index) => {
        ...
        const categorySubgroupNode = createCategorySubgroupNode(
          selection,
          field,
          group,
          index,
      ...
      encodingNode.children = values.map((datum, index) =>
        createDatumNode(datum, index, field, encodingNode, spec)
      );
    }
    return encodingNode;
  };

  const createNumericSubgroupNode = (
    group,
    field,
    extent,
    index,
    ...
    const node = {
      ...
      selection: group,
      lastVisitedChild: null,
      description: numericSubgroupDesc(
        group,
        field,
        extent,
        index,
  ...
  const createNumericEncodingNode = (
    groupSize,
    domain,
    type,
    title,
    ...
    encodingNode.type = "category-group";
    encodingNode.description = numericGroupDesc(
      type,
      title,
      domain,
      field,
    ...
    encodingNode.children = d3.range(groupSize).map((group, groupIdx) => {
      ...
      return createNumericSubgroupNode(
        selection,
        field,
        extent,
        groupIdx,
  ...
  rootNode.children = scales
    .filter((d) => !skip || !skip.includes(d.name))
    .map((d) => {
      ...
      switch (d.type) {
        ...
        case "linear":
          ...
          return createNumericEncodingNode(
            groupSize,
            domain,
            type,
            title,
        ...
        case "band":
          ...
          title = getEncodingTitle(spec, d.name);
          field = getFieldName(spec, d.name); // find the data field name
          return createCategoryEncodingNode(
            domain,
            type,
            title,
            field,
